      ...
      <div className={`task-card ${isExpired ? 'expired' : ''}`}>
          ...
          <div className="task-meta-info">
            ...
            <div className="task-points">
              {task?.dynamic ? (
                ...
                <span>{task?.points || 0} Punkte</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="task-submission">
          ...
          <div className="file-upload-section">
            <label className="file-upload-label">
              <input
                ...
                style={{ display: 'none' }}
                disabled={userSubmissionsCount >= (task?.max_submissions || 0) || isSubmitting || isExpired}
              />
              {!selectedFile ? (
                <div className={`upload-placeholder ${userSubmissionsCount >= (task?.max_submissions || 0) || isExpired ? 'disabled' : ''}`}>
                  <FaUpload />
          ...
          <button
            ...
            disabled={!selectedFile || (task?.dynamic && !dynamicValue) || userSubmissionsCount >= (task?.max_submissions || 0) || isSubmitting || isExpired}
          >
            {isSubmitting ? <FaSpinner className="spin" /> : (isExpired ? 'Abgelaufen' : (userSubmissionsCount >= (task?.max_submissions || 0) ? 'Maximale Anzahl erreicht' : 'Aufgabe einreichen'))}
          </button>
        </div>
      </div>
    );
